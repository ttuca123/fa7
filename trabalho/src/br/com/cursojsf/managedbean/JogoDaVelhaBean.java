...
public class JogoDaVelhaBean {
	...
	private void limparBotoes() {
		UIComponent painel = FacesContext.getCurrentInstance().getViewRoot().findComponent("usuarioForm").findComponent("painelDeBotoes");
	...
	private Character verificarLinhas() {
		for(int i=0; i <3; i++) {
			if(jogadas[i][0] == jogadas[i][1] && jogadas[i][1] == jogadas[i][2] && jogadas[i][2] != null) {
				return jogadas[i][0];
			}
		}
		return null;
	...
	private Character verificarColunas() {
		for(int i=0; i <3; i++) {
			if(jogadas[0][i] == jogadas[1][i] && jogadas[1][i] == jogadas[2][i] && jogadas[2][i] != null) {
				return jogadas[0][i];
			}
		}
		return null;
	...
	private Character verificarDiagonais() {
		if(jogadas[0][0] == jogadas[1][1] && jogadas[1][1] == jogadas[2][2] && jogadas[2][2] != null) {
			return jogadas[0][0];
		}
		if(jogadas[0][2] == jogadas[1][1] && jogadas[1][1] == jogadas[2][0] && jogadas[2][0] != null) {
			return jogadas[0][0];
		}
		return null;
	}
	...
	private void atualizarTitulo() {
		titulo = String.format("É a vez de “%s” jogar", jogadorAtual);
	}
